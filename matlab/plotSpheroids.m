...
%   intersections - Coordinates of M intersections (2xM)
    ...
    for ii = 1:numSpheroids
        ...
        plots(end+1) = plot3(spheroid(1,:), spheroid(2,:), spheroid(3,:),'.','MarkerSize',10);
        legendArray{end+1} = "Spheroid "+num2str(ii);
        % Plot Rx->Tx pairs
        plots(end+1)=plot3([sharedFocalPoint(1), otherFocalPoints(1,ii)], ...
    ...
    if numIntersections == 1
            plots(end+1) = plot3(intersections(1), intersections(2), intersections(3), ...
    ...
    elseif numIntersections > 1
        for jj=1:size(intersections,2)
            ...
            plots(end+1) = plot3(intersections(1,jj), intersections(2,jj), intersections(3,jj), ...
