...
%   intersections - Coordinates of M intersections (2xM)
    ...
    plots(end+1) = plot(sharedFocalPoint(1),sharedFocalPoint(2),'Marker','o','MarkerSize',10,'Color','k');
    legendArray{end+1} = "Shared focal point";

    % Plot non-shared focal points
    for ii=1:numEllipses
        plots(end+1) = plot(otherFocalPoints(1,ii),otherFocalPoints(2,ii),'Marker','*','MarkerSize',10,'Color',rgb(mod(ii,3)+1));
        legendArray{end+1} = "focal point #" + num2str(ii);
    end

    % Plot ellipses
    for ii=1:numEllipses
        plots(end+1) = plot(ellipseCoord(1,:,ii), ellipseCoord(2,:,ii),'-','Color',rgb(mod(ii,3)+1));
    ...
    for jj=1:size(intersections,2)
        ...
        plots(end+1) = plot(intersections(1,jj), intersections(2,jj),'Marker',mkr,'MarkerSize',12,'Color','k','LineStyle',':');
    ...
    axis equal
    ax = gca;
    ax.XTick = limX(1):limX(2);
    ax.YTick = limY(1):limY(2);
    title(titleText)
    legend(plots,legendArray);
